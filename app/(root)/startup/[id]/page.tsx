...
const page = async ({ params }: { params: { id: string } }) => {
  ...
  const postDetails = await db.select().from(startups).where(eq(startups.id, numericId)).innerJoin(users,eq(startups.userId, users.id));
      ...
      <div>
        ...
        <section className='p-8'>
          <div className='flex justify-center items-center w-full p-4'>
            <Image src={postImage} alt='pitch image' height={450} width={450} className='rounded-xl'/>
          </div>
          <div className='p-3 flex items-center gap-2 bg-slate-300 rounded-xl border border-black md:p-5 md:w-[400px]'>
            <Image src={avatarImage} alt='user_photo' width={45} height={45} className='rounded-full flex flex-col md:flex-row'/>
            <div>
